...
### FK(Foreign Key) 사용하지 않음

**결정일**: 2026-02-10

**배경**: FK가 데이터 정합성을 보장하지만, 그것이 논리적으로 맞는다는 보장은 아님. DB 스키마 제약보다 데이터 자체를 중요시하는 방향.

**결정**:
- 외래키(REFERENCES) 사용하지 않음
- CASCADE 사용하지 않음
...
**배경**: 모든 테이블 PK가 `id`이면 JOIN이나 코드에서 혼동 발생.

**결정**:
- 새 테이블부터 PK를 `extraction_id`, `link_id` 등 명시적 이름 사용
- 기존 테이블은 변경하지 않음
