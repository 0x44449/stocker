...
/**
 * KRX 투자자별 거래실적 CSV 다운로드 클라이언트.
 */
@Component
public class KrxInvestorFlowCsvDownloader {
    ...
    public KrxInvestorFlowCsvDownloader() {
    ...
    public byte[] download(KrxSession session, String otp) {
        crawlLog.info("event=KRX_INVESTOR_CSV_DOWNLOAD_START");
        ...
        try {
            ...
            if (csvBytes == null || csvBytes.length == 0) {
                crawlLog.warn("event=KRX_INVESTOR_CSV_DOWNLOAD_FAILED reason=EMPTY_RESPONSE");
                throw new KrxInvestorFlowException(
                        KrxInvestorFlowException.ErrorType.CSV_DOWNLOAD_FAILED,
                        "Empty CSV response"
                );
            }

            crawlLog.info("event=KRX_INVESTOR_CSV_DOWNLOAD_SUCCESS bytes={}", csvBytes.length);
            return csvBytes;

        } catch (KrxInvestorFlowException e) {
            throw e;
        } catch (Exception e) {
            crawlLog.warn("event=KRX_INVESTOR_CSV_DOWNLOAD_FAILED reason=HTTP_ERROR");
            throw new KrxInvestorFlowException(
                    KrxInvestorFlowException.ErrorType.CSV_DOWNLOAD_FAILED,
